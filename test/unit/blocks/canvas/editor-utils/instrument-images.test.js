...
// Build a doc with (i) a standalone image (sole child of a paragraph) and
// (ii) an inline image (paragraph with text + image). Standalone images lose
// their <p data-prose-index> wrapper during prose2aem's makePictures, which is
// the bug data-image-index fixes.
...
describe('getInstrumentedHTML image instrumentation', () => {
  ...
  it('stamps every content image with a data-image-index that resolves to the image node', () => {
    ...
    // Both the standalone and inline images must be stamped.
    ...
    stamped.forEach((img) => {
      ...
      // CRITICAL: the stamped index must be the image node's PM `from`, so the
      // parent's doc.nodeAt(idx) resolves back to the image (how the iframe and
      // handleNodeSelect consume it).
