...
// Mirrors the proven table shape in edit/prose/plugins/tableUtils.test.js:
// a single-cell block header (colspan 2). First-cell text is the block name.
...
describe('handleNodeSelect', () => {
  ...
  it('ignores a payload whose type does not match the resolved node', () => {
    const tablePos = insertTable(editor);
    const ctx = { view: editor.view };
    // Claiming "image" at a table position must be a no-op, not a throw.
...
describe('resolveNodeSelectPos', () => {
  ...
  it('falls back to a src lookup (by filename) when the image proseIndex is stale', () => {
    ...
    const { doc } = editor.view.state;
    // Stale/out-of-range index, but the transform-suffixed src matches by filename.
